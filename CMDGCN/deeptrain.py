...
from deepmodel import Encoder, Model, drop_feature, SelfExpr, ClusterModel
...
def batch_cluster_train(x, edge_index, from_list, to_list, val_list,
    ...
    for epoch in range(max_epoch+1):
        ...
        # # # 给loss1单独加上正则化项
        # loss1 = F.mse_loss(pred_similarity, val_list)+ 0.1*sum(torch.norm(p ,2)**2 for p in clustermodel.parameters())
...
if __name__ == '__main__':
    ...
    # MODEL_PATH = args.path + "/Saved_Models/" + "Cardiotocography" + "/"
    cfg = yaml.load(open(args.config), Loader=SafeLoader)[args.dataset]
    # cfg = yaml.load(open(args.config), Loader=SafeLoader)['Cardiotocography']
    ...
    encoder1 = Encoder(num_features//4, cfg['num_hidden'], activation,
                      base_model=base_model, k=cfg['num_layers']).to(device)
    encoder2 = Encoder(num_features// 4 + cfg['num_hidden'], cfg['num_hidden']//2, activation,
                       base_model=base_model, k=cfg['num_layers']).to(device)
    encoder3 = Encoder(num_features // 4 + cfg['num_hidden']//2, cfg['num_hidden']//2, activation,
                       base_model=base_model, k=cfg['num_layers']).to(device)
    encoder4 = Encoder(num_features-(num_features // 4*3) + cfg['num_hidden']//2, cfg['num_hidden'], activation,
                       base_model=base_model, k=cfg['num_layers']).to(device)
    encoder5 = Encoder(num_features + cfg['num_hidden'], cfg['num_hidden'], activation,
                       base_model=base_model, k=cfg['num_layers']).to(device)
    gracemodel = Model(encoder1,encoder2,encoder3,encoder4, encoder5,cfg['num_hidden'], cfg['num_proj_hidden'], cfg['tau']).to(device)
