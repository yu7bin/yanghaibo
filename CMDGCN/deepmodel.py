...
class Model(torch.nn.Module):
    def __init__(self, encoder1: Encoder,encoder2: Encoder,encoder3: Encoder,encoder4: Encoder,encoder5: Encoder, num_hidden: int, num_proj_hidden: int,
                 tau: float = 0.5):
    ...
    def forward(self, x: torch.Tensor,
        ...
        mid_dim = x.size(1) // 4
        h = x[:, :mid_dim]  # 提取前一半特征
        residual1 = x[:, mid_dim:2*mid_dim]  # 提取后一半特征
        residual2 = x[:, 2*mid_dim:3*mid_dim]
        residual3 = x[:, 3*mid_dim:]
        # 将 h 通过 encoder 处理
        h = self.encoder1(h, edge_index)
        # 将处理后的 h 和原始的 residual 特征在列上进行拼接
        h = torch.cat((h, residual1), dim=1)
        # 再次将拼接后的特征通过 encoder 处理
        h = self.encoder2(h, edge_index)
        h = torch.cat((h, residual2), dim=1)
        h = self.encoder3(h, edge_index)
        h = torch.cat((h, residual3), dim=1)
        h = self.encoder4(h, edge_index)
        h = torch.cat((h, x), dim=1)
        h = self.encoder5(h, edge_index)
